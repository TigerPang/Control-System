...
{ 
  ...
  if (loop_number >= update_rate) // only report values back to host every 'update_rate' loop
  {
    ...
    for (int i = 0; i <= 4; i++) // print data to serial monitor
    {
      Serial.print(IMU_data[i], 6); // prints data using 6 decimal places
      Serial.print(" "); // seperate data with space
    }
    Serial.print(IMU_data[5], 6); // print last data point without space
...
void Get_IMU_Data()
{
  ...
  Acc_scale = 1; // calculate normalisation factor. Put normalisation formula in here
  for (int i = 0; i <= 2; i++)
  {
  ...
  for (int i = 0; i <= 2; i++)
  {
    ...
    {
      Gyro_past[i] = Gyro_past[i]; // filter the gyro measurement to estimate offset for drift removal. Put low-pass filter formula in here
    }
